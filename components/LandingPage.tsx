...
const copy = {
  ...
  en: {
    ...
    voiceKicker:"HAY VOICE / HY-AM", voiceTitle:"Armenian should sound Armenian.", voiceText:"HAY Voice is provider-agnostic: ElevenLabs custom voices + Azure Anahit/Hayk fallback + future Gemini/voice-clone layers. Text first passes pronunciation, numbers, code-switch and conversational naturalization.", voiceCta:"Open Voice Lab",
...
export default function LandingPage(){
  ...
  return <main className="landingPage">
    ...
    <section className="landingHero">
      <div className="landingHeroCopy"><span className="landingEyebrow"><i/>{t.eyebrow}</span><h1><small>{t.heroA}</small>{t.heroB}</h1><p>{t.sub}</p><div className="landingActions"><a className="landingPrimary" href="/studio?plan=free">{t.primary}</a><a className="landingSecondary" href="#product">{t.secondary} ↓</a></div></div>
    ...
    <section id="voice" className="voiceSection"><div className="voiceCopy"><span>{t.voiceKicker}</span><h2>{t.voiceTitle}</h2><p>{t.voiceText}</p><div className="landingActions"><a className="landingSecondary" href="/voice">{t.voiceCta} ↗</a></div><div className="voiceModeList">{t.voiceModes.map((mode,i)=><div key={mode}><i>{String(i+1).padStart(2,"0")}</i><b>{mode}</b><span>HY-AM</span></div>)}</div></div><div className="voiceWave" aria-hidden="true"><div className="waveBars">{Array.from({length:72},(_,i)=><i key={i} style={{height:`${18+((i*37)%82)}%`}}/>)}</div><div className="voiceQuote">«Բարև, հիմա ցույց տամ՝ HAY-ը ոնց է քո բիզնեսի համար ամբողջ շաբաթվա կոնտենտը կառուցում»</div><div className="voiceMeta"><span>NATURAL EASTERN ARMENIAN</span><b>խոսակցական / conversational</b></div></div></section>

    <section id="pricing" className="landingSection pricingSection"><div className="sectionIntro"><span>{t.pricingKicker}</span><h2>{t.pricingTitle}</h2></div><div className="pricingGrid">{HAY_PLANS.map(plan=><article key={plan.id} className={plan.id==="growth"?"featured":""}>{plan.badge&&<em>{plan.badge}</em>}<div className="planTop"><span>HAY / {plan.name.toUpperCase()}</span><h3>{plan.name}</h3><p>{plan.description[locale]}</p></div><div className="planPrice"><b>{amd(plan.priceAmd)}</b><span>{t.perMonth}</span></div><ul>{plan.features[locale].map(feature=><li key={feature}>↗ {feature}</li>)}</ul><a href={`/studio?plan=${plan.id}`}>{t.choose} →</a></article>)}</div><div className="agencyBand"><div><span>{t.agency}</span><h3>{t.from} {amd(AGENCY_STARTING_AMD)} / {locale==="hy"?"ամիս":locale==="ru"?"месяц":"month"}</h3></div><p>{t.agencyText}</p><a href="/studio?plan=agency">CONTACT →</a></div><p className="pricingNote">AI video credits protect predictable pricing because premium generative video is usage-based. Extra generation can be purchased as add-on credits.</p></section>
