...
const channels: Array<{ platform: SocialPlatform; label: string }> = [
  { platform:"instagram", label:"Instagram" }, { platform:"tiktok", label:"TikTok" },
  { platform:"youtube", label:"YouTube" }, { platform:"facebook", label:"Facebook" },
];
...
export default function MarketingOS() {
  ...
  async function waitForRender(item:ContentItem,jobId:string){
    for(let attempt=0;attempt<120;attempt++){
      ...
      try{
        ...
        if(attempt>5){setAssetState(state=>({...state,[item.id]:"error"}));setMessage(error instanceof Error?error.message:"Render status failed");return;}
      }
    }
    setAssetState(state=>({...state,[item.id]:"rendering"}));
    setMessage("Render is still processing. HAY will keep the durable render job in the project history.");
  }
  ...
  async function createAsset(item:ContentItem){
    ...
    try{
      ...
      if(jobId){setRenderJobs(state=>({...state,[item.id]:jobId}));setAssetState(state=>({...state,[item.id]:"rendering"}));setMessage(`Render job ${jobId} started.`);void waitForRender(item,jobId);return;}
  ...
  const assetLabel=(item:ContentItem)=>{const state=assetState[item.id];if(state==="creating")return"Creating ···";if(state==="rendering")return"Rendering ···";if(state==="ready")return"Publish →";if(state==="renderable")return"Render setup";if(state==="setup")return"Setup needed";if(state==="error")return"Retry";return"Create asset →";};

  return <main className="marketingPage">
    <header className="marketingNav"><a href="/" className="logoLink"><HayLogo/></a><nav className="productNav"><a className="active" href="/studio">Marketing OS</a><a href="/creator">Creator</a><a href="/voice">Voice</a><a href="/developers">Language API</a></nav><div className="navTools"><a href="/login" className="accountLink">ACCOUNT</a><div className="localePill">{(["hy","en","ru"] as Locale[]).map(item=><button key={item} className={locale===item?"active":""} onClick={()=>{setLocale(item);patchBusiness("primaryLanguage",item);}}>{item==="hy"?"ՀԱՅ":item.toUpperCase()}</button>)}</div><button className="commandButton">⌘ K</button></div></header>
    ...
    <section className="marketingGrid">
      ...
      <article className="intelCard channelsCard"><div className="panelTop"><span>02 / {t.connections.toUpperCase()}</span><em>OAUTH</em></div><div className="channelList">{channels.map(({platform,label})=>{const state=connectionState[platform]||"disconnected";return <button key={platform} onClick={()=>connect(platform)}><i><SocialBrandIcon platform={platform} size={18} decorative/></i><div><strong>{label}</strong><small>{state==="setup"?"developer app required":state}</small></div><span className={`connectionLed ${state}`}/></button>;})}</div><p className="microcopy">Account owner authorizes every channel. HAY never asks for social passwords.</p></article>
