...
type ExperimentRun={id:string;campaignId:string;campaignName?:string;variable:string;primaryMetric:string;status:string;scheduledFor:string;control:{id:string;value:number;measuredAt?:string;status?:string|null;hook?:string};variant:{id:string;value:number;measuredAt?:string;status?:string|null;hook?:string};winner?:"control"|"variant"|"tie"|null;lift?:number|null};
type RunsResponse={configured:boolean;error?:string;runs?:ExperimentRun[]};
...
const liftLabel=(value?:number|null)=>value===null||value===undefined?"—":`${value>=0?"+":""}${Math.round(value*100)}%`;
...
export default function StudioCampaignAnalytics(){
  ...
  async function loadRuns(campaignId?:string){
    if(!campaignId){setRuns([]);return;}
    try{
      const response=await fetch(`/api/studio/experiments?campaignId=${encodeURIComponent(campaignId)}`,{cache:"no-store"});
      if(response.status===401){setRuns([]);return;}
      const result=await response.json() as RunsResponse;setRuns(result.runs||[]);
    }catch{setRuns([]);}
  }
  ...
  async function load(campaignId?:string){
    ...
    try{
      ...
      if(response.status===401){setData({configured:true,error:"unauthorized",campaigns:[]});setRuns([]);return;}
      const next=await response.json() as Response;setData(next);
      const resolved=campaignId||next.analytics?.campaignId||"";
      if(resolved)setSelected(resolved);
      await loadRuns(resolved||undefined);
    }catch{setData({configured:false,error:"unavailable",campaigns:[]});setRuns([]);}
  ...
  async function changeCampaign(id:string){setSelected(id);setRunnerMessage("");await load(id);}
  async function runExperiment(){
    const analytics=data?.analytics;const control=analytics?.topContent?.[0];
    if(!analytics?.experiment.ready||!control||runnerBusy)return;
    setRunnerBusy(true);setRunnerMessage("");
    try{
      const response=await fetch("/api/marketing/experiment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({campaignId:analytics.campaignId,sourceContentId:control.id,variable:"hook"})});
      if(response.status===401){window.location.href="/login";return;}
      const result=await response.json();
      if(!response.ok){
        if(response.status===409&&result.experiment){setRunnerMessage("A controlled hook test already exists for this control asset.");await loadRuns(analytics.campaignId);return;}
        throw new Error(result.detail||result.error||"experiment_create_failed");
      }
      setRunnerMessage(`Variant created as draft · ${result.variant?.scheduledFor?new Date(result.variant.scheduledFor).toLocaleString():"comparable window prepared"}. Review it before publishing.`);
      await loadRuns(analytics.campaignId);window.dispatchEvent(new Event("hay:studio-refresh"));
    }catch(error){setRunnerMessage(error instanceof Error?error.message:"Experiment creation failed");}
    ...
    {loading?<div className="campaignAnalyticsEmpty"><strong>Reading campaign evidence…</strong></div>:
      ...
      <>
        ...
        <article className={`experimentBrain ${analytics.experiment.ready?"ready":"waiting"}`}>
          <div className="experimentLabel"><span>03 / NEXT CONTROLLED EXPERIMENT</span><div><b>{analytics.experiment.ready?"READY TO TEST":"WAITING FOR EVIDENCE"}</b>{analytics.experiment.ready&&<button onClick={()=>void runExperiment()} disabled={runnerBusy||!analytics.topContent[0]}>{runnerBusy?"CREATING…":"RUN CONTROLLED TEST →"}</button>}</div></div>
          ...
          {latestRun&&<div className={`experimentRunStatus run-${latestRun.status}`}>
            <div><span>LIVE TEST</span><strong>{latestRun.variable.toUpperCase()} · {latestRun.status.toUpperCase()}</strong><small>Primary metric · {latestRun.primaryMetric.toUpperCase()}</small></div>
            <div><span>CONTROL</span><strong>{fmt(latestRun.control.value)}</strong><small>{latestRun.control.hook||latestRun.control.id}</small></div>
            <div><span>VARIANT</span><strong>{fmt(latestRun.variant.value)}</strong><small>{latestRun.variant.hook||latestRun.variant.id}</small></div>
            <div><span>RESULT</span><strong>{latestRun.winner?`${latestRun.winner.toUpperCase()} · ${liftLabel(latestRun.lift)}`:"WAITING"}</strong><small>{latestRun.status==="draft"?"Variant is waiting for human review.":latestRun.status==="running"?"Variant published; waiting for both metric snapshots.":"Winner is descriptive, not proof of causality."}</small></div>
          </div>}
          {runnerMessage&&<p className="experimentRunnerMessage">{runnerMessage}</p>}
        </article>
