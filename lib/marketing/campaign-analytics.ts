...
export type CampaignExperiment={ready:boolean;reason?:string;hypothesis?:string;control?:string;variant?:string;holdConstant?:string[];primaryMetric?:string;caveat?:string};
export type CampaignAnalytics={
  campaignId:string;campaignName:string;primaryKpi:string;totalItems:number;measuredItems:number;coverage:number;totals:CampaignMetricTotals;
  phasePerformance:CampaignPhaseAnalytics[];topContent:CampaignAnalyticsContent[];
  ...
  primarySignal:{metric:string;value:number;label:string};evidenceQuality:"none"|"early"|"partial"|"strong";experiment:CampaignExperiment;caveats:string[];
};
...
type GroupSignal={name:string;items:number;totals:CampaignMetricTotals;score:number};
...
function evidenceQuality(total:number,measured:number):CampaignAnalytics["evidenceQuality"]{if(!measured)return "none";const coverage=measured/Math.max(1,total);if(measured<3)return "early";if(coverage<.7)return "partial";return "strong";}
function aggregate(items:CampaignAnalyticsContent[],key:"format"|"platform",kpi:string):GroupSignal[]{
  ...
  return [...groups.entries()].map(([name,value])=>({name,items:value.items,totals:value.totals,score:score(value.totals,kpi)/Math.max(1,value.items)})).sort((a,b)=>b.score-a.score);
}
function experimentFor(items:CampaignAnalyticsContent[],formats:Array<{format:string;items:number;score:number}>,kpi:string):CampaignExperiment{
  ...
  const ranked=[...items].sort((a,b)=>score(b.metrics,kpi)-score(a.metrics,kpi));const leader=ranked[0];const challenger=ranked.find(item=>item.id!==leader.id);
  if(!challenger)return {ready:false,reason:"Not enough distinct measured assets for a comparison."};
  const bestFormat=formats[0]?.format||leader.format;const alternative=formats.find(row=>row.format!==bestFormat)?.format||challenger.format;
  const primary=kpi==="conversion"?"conversions":kpi==="trust"?"saves":kpi==="community"?"comments":kpi==="reach"?"reach":"conversions";
  return {ready:true,hypothesis:`The ${bestFormat} creative pattern may outperform ${alternative} for this campaign objective; test it under controlled conditions rather than treating the observed difference as causal.`,control:`Reuse the weaker pattern as control: ${challenger.platform} / ${challenger.format} / “${challenger.hook.slice(0,100)}”.`,variant:`Test the stronger pattern: ${leader.platform} / ${bestFormat} / a new hook with the same offer and audience.`,holdConstant:["offer","audience segment","campaign phase","CTA intent","publish window as closely as practical"],primaryMetric:primary,caveat:"This recommendation is based on observational campaign data. A controlled follow-up is required before claiming causality."};
}
...
export function buildCampaignAnalytics(args:{campaign:CampaignInput;content:ContentInput[];metrics:MetricInput[]}):CampaignAnalytics{
  ...
  const allIds=[...new Set([...phaseByContent.keys()])];const contentById=new Map(args.content.map(row=>[text(row.id),row]));const latest=new Map<string,MetricInput>();
  ...
  for(const [name,meta] of phaseMeta){const phaseItems=measured.filter(item=>item.phase===name);const phaseTotals=empty();for(const item of phaseItems)add(phaseTotals,item.metrics);phasePerformance.push({name,label:meta.label,totalItems:meta.ids.length,measuredItems:phaseItems.length,coverage:meta.ids.length?phaseItems.length/meta.ids.length:0,totals:phaseTotals,score:phaseItems.length?score(phaseTotals,kpi)/phaseItems.length:0});}
  const formatSignals=aggregate(measured,"format",kpi);const platformSignals=aggregate(measured,"platform",kpi);
  const formats=formatSignals.map(item=>({format:item.name,items:item.items,totals:item.totals,score:item.score}));
  const platforms=platformSignals.map(item=>({platform:item.name,items:item.items,totals:item.totals,score:item.score}));
  ...
  return {campaignId,campaignName,primaryKpi:kpi,totalItems,measuredItems,coverage:totalItems?measuredItems/totalItems:0,totals,phasePerformance,topContent,formatPerformance:formats,platformPerformance:platforms,primarySignal:primarySignal(totals,kpi),evidenceQuality:quality,experiment:experimentFor(measured,formats,kpi),caveats:["Campaign analytics use the latest stored metric snapshot per content item.","Format/platform/phase ranking scores are internal directional indices normalized per measured asset; they are not business KPIs.","Observed differences are correlations, not proof that a hook, format or phase caused the result.",...(kpi==="retention"?["HAY does not currently have a dedicated retention event in content_metrics; displayed outcome metrics are proxies only."]:[]),...(quality!=="strong"?["Evidence is incomplete; interpret rankings as directional until more campaign items have measured snapshots."]:[])]};
}
