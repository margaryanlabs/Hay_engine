import "server-only";
import { effectiveOutcome, loadAttributionSummary } from "./attribution";
...
export type MarketingPerformanceContext = {
  ...
  firstPartyAttribution?: { available:boolean; trackedClicks:number; trackedConversions:number };
};
...
export async function loadMarketingPerformance(businessId?: string, businessName?: string): Promise<MarketingPerformanceContext | null> {
  ...
  const [{ data: snapshots }, { data: content }, attribution] = await Promise.all([
    ...
    loadAttributionSummary(supabase,resolvedBusinessId),
  ]);
  ...
  const measuredIds = new Set([...latest.keys(), ...attribution.byContent.keys()]);
  if (!measuredIds.size) return null;
  ...
  for (const id of measuredIds) {
    ...
    const outcome=effectiveOutcome(number(row.clicks),number(row.conversions),attribution.byContent.get(id));
    totals.views += number(row.views); totals.reach += number(row.reach); totals.likes += number(row.likes); totals.comments += number(row.comments); totals.shares += number(row.shares); totals.saves += number(row.saves); totals.clicks += outcome.clicks; totals.conversions += outcome.conversions;
  ...
  return {
    measuredPosts: measuredIds.size,
    totals,
    ...
    firstPartyAttribution:{available:attribution.available,trackedClicks:attribution.totals.clicks,trackedConversions:attribution.totals.conversions},
  };
}
